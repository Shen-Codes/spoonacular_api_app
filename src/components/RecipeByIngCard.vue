<template>
  <div class="recipe-by-ing-card">
    ...
    <p
      class="recipe-by-ing-card__ingredient"
      v-for="ingredient in allIngredients"
      :key="ingredient.id"
    >
      {{ ingredient.name
      }}<span>{{ ingredient.missed ? " - missing" : "" }}</span>
    </p>
    <h4 class="recipe-by-ing-card__h4" v-if="unusedExists">
      Unused Ingredients
    </h4>
    <p
      class="recipe-by-ing-card__ingredient"
      v-for="ingredient in unusedIngredients"
      :key="ingredient.id"
    >
      {{ ingredient.name }}
    </p>
    <div class="recipe-by-ing-card__button">
      <Button type="Get Full Recipe" @click="(e) => handleGetRecipe(id, e)" />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...mapActions(["getFullRecipe", "modalSwitch"]),
    handleGetRecipe(id, e) {
      e.preventDefault();
